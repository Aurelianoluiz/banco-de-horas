...
const readBody = async (req) => {
  ...
  if (declared > MAX_BODY_BYTES) throw Object.assign(new Error('Payload muito grande'), { statusCode: 413 });
  const chunks = []; let size = 0;
  for await (const chunk of req) {
    size += chunk.length;
    if (size > MAX_BODY_BYTES) throw Object.assign(new Error('Payload muito grande'), { statusCode: 413 });
    chunks.push(chunk);
  }
...
const server = createServer(async (req, res) => {
  try {
    if (req.url?.startsWith('/api/')) {
      ...
      res.writeHead(result.status || 200, { ...securityHeaders, ...(result.headers || {}) }); res.end(result.body); return;
    }
    ...
    res.end(JSON.stringify({ error: status === 413 ? 'Payload muito grande' : 'Erro interno do servidor' }));
  }
});

const shutdown = async (signal) => { server.close(async () => { await pool.end(); process.exit(signal === 'SIGINT' ? 130 : 143); }); };
