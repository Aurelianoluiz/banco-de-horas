...
export const createPostgresAuth = ({ repository, tokenSecret } = {}) => {
  ...
  const login = async ({ email, senha }) => {
    ...
    const result = await repository.query(
      'SELECT id, nome, senha_hash, perfil, ativo FROM usuarios WHERE lower(email) = lower($1) LIMIT 1',
      [email]
    );
    const user = result.rows[0];
    if (!user || !user.ativo || !verifyPassword(senha, user.senha_hash)) return null;
    const role = normalizeRole(user.perfil);
    if (!permissions[role]) return null;
    return {
      token: sign({ sub: user.id, role, exp: Date.now() + 8 * 60 * 60 * 1000 }, tokenSecret),
      user: { id: user.id, nome: user.nome, role }
    };
  };

  return {
    login,
    authenticate: async (token) => verifyToken(token, tokenSecret),
    authorize: async (identity, resource, action) => {
      if (!identity) return false;
      const role = permissions[normalizeRole(identity.role)] || {};
...
export { hashPassword, verifyPassword, normalizeRole };
