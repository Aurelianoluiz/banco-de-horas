...
function Read-Asset([string]$FilePath) {
  ...
  if ([IO.Path]::GetExtension($FilePath).ToLowerInvariant() -eq '.html') {
    ...
    if ($FilePath -ieq (Join-Path $Root 'index.html')) {
      $tag = '<script src="/web/index-compat.js"></script>'
      if ($text -notmatch 'web/index-compat.js') { $text = $text -replace '(?i)</head>', "$tag</head>" }
    }
