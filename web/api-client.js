...
const sessionToken = () => { try { return sessionStorage.getItem('bh.auth.token'); } catch { return null; } };
const authHeaders = () => { const value = sessionToken(); return value ? { authorization: `Bearer ${value}` } : {}; };
...
const binaryRequest = async (path) => {
  const response = await fetch(`/api/${path}`, { credentials: 'same-origin', headers: authHeaders() });
  if (!response.ok) {
    const data = await response.json().catch(() => null);
    const error = new Error(data?.error || `HTTP ${response.status}`);
    error.status = response.status;
